...
export async function matchV5TimelineLatestMatchExample() {
	...
	const summoner = (await api.Summoner.getByName(summonerName, Regions.EU_WEST))
		.response;
	console.log("Found summoner:", summoner.puuid);

	const matchlist = (
		await api.MatchV5.list(summoner.puuid, RegionGroups.EUROPE, { queue: 450 })
	).response;
	...
	const timeline = (await api.MatchV5.timeline(matchId, RegionGroups.EUROPE))
		.response;
