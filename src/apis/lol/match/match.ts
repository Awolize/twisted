...
export class MatchApi extends BaseApiLol {
	...
	private generateResponse(
		error: GenericError,
	): ApiResponseDTO<MatchListingDto> {
		return {
	...
	private map(match: ApiResponseDTO<MatchDto>) {
		...
		match.response.remake = match.response.teams.every(
			(t: MatchTeamsDto) => !t.firstTower,
		);
		return match;
	}
	/**
	...
	public async get(matchId: number, region: Regions) {
		...
		const data = await this.request<MatchDto>(
			region,
			endpointsV4.Match,
			params,
		);
	...
	public async list(
		...
		try {
			return await this.request<MatchListingDto>(
				region,
				endpointsV4.MatchListing,
				params,
				false,
				query,
			);
	...
	public async timeline(matchId: number, region: Regions) {
		...
		return this.request<MatchTimelineDto>(
			region,
			endpointsV4.MatchTimeline,
			params,
		);
	}
}
