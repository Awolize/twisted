...
export class ClashApi extends BaseApiLol {
	...
	getTeamById(teamId: string, region: Regions) {
		return this.request<ClashTeamDto>(region, endpointsClashV1.GetTeam, {
			teamId,
		});
	}
	/**
	...
	getTournaments(region: Regions) {
		return this.request<TournamentClashDTO[]>(
			region,
			endpointsClashV1.GetTournaments,
		);
	}
	/**
	 * Get team tournament
	...
	getTeamTournament(teamId: string, region: Regions) {
		return this.request<TournamentClashDTO>(
			region,
			endpointsClashV1.GetTournamentByTeamId,
			{ teamId },
		);
	}
	/**
	...
	getTournamentById(tournamentId: string | number, region: Regions) {
		return this.request<TournamentClashDTO>(
			region,
			endpointsClashV1.GetTournamentById,
			{ tournamentId },
		);
	}
}
